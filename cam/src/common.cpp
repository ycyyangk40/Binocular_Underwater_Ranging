...
bool openStereoCamera(cv::VideoCapture& cap, int deviceId, int width, int height, int fps) {
#ifdef _WIN32
...
#endif
	if (!cap.isOpened()) {
		std::cerr << "Failed to open camera " << deviceId << "\n";
		return false;
	}
	...
	std::cout << "Camera " << deviceId << " opened: " << w << "x" << h
	          << " FPS=" << f
	          << " FOURCC=" << fourcc << "\n";

	cv::Mat testFrame;
	if (!cap.read(testFrame) || testFrame.empty()) {
		std::cerr << "Camera opened but first frame read failed.\n";
		return false;
	}
	std::cout << "First frame size: " << testFrame.cols << "x" << testFrame.rows << "\n";
	return true;
}
